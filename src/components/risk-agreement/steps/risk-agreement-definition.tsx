import {Button, CircularProgress, DialogActions, DialogContentText} from "@mui/material";
import React, {useState} from "react";
import {Chat, ChatMessage} from "../../../store/slices/my-bids/types";
import {useDispatch, useSelector} from "react-redux";
import {selectActiveChat, selectActiveMessages} from "../../../store/slices/my-bids/selectors";
...
export const RiskAgreementDefinition = (props: RiskAgreementDefinitionProps) => {
    const activeChat: Chat | undefined = useSelector(selectActiveChat);
    const [loadingExtractInformation, setLoadingExtractInformation] = useState(false);
    ...
    const handleRiskAgreementProposition = () => {
        ...
        const riskGiverApprovals = isRiskTaker // if riskTaker, riskGiver has to approve and vice versa
            ? {costs: false, insuranceSum: false, timeframe: false, evidence: false, details: false}
            : {costs: true, insuranceSum: true, timeframe: true, evidence: true, details: true};
        ...
        handleNotificationsUpdate()
            .then(() => {
                showSnackbar(
                    ...
                    `Dein Vorschlag wurde an ${activeChat?.riskTaker.uid === auth?.currentUser?.uid ? activeChat?.riskProvider.name : activeChat?.riskTaker.name} gesendet.`,
                    {vertical: "top", horizontal: "center"},
                    "success"
                );
            })
