...
export const QuickMenuButtons = (props: AuthenticationButtonsProps) => {
    ...
    const markAsRead = async (notificationId: string) => {
        const notificationsRef = doc(db, `notifications/${userId}/notifications/${notificationId}`);
        await updateDoc(notificationsRef, {status: NotificationStatusEnum.READ});

        setNotifications((prev) =>
            prev.map((notif) => (notif.id === notificationId? { ...notif, status: NotificationStatusEnum.READ} : notif))
        );
    };
                ...
                props.isLoggedIn ? (
                    <Box sx={{flexGrow: 0}}>
                        <IconButton
                            ...
                            <Badge badgeContent={notifications.filter((n) => n.status === NotificationStatusEnum.UNREAD).length} color="error">
                                <NotificationsIcon />
                            </Badge>
                        </IconButton>
                        {/* Notification Menu */}
                    ...
                    <Menu
                        ...
                        PaperProps={{
                            style: { maxHeight: 500, width: "450px", padding: "8px", overflowX: "auto",},
                        }}
                    >
                        {/* Header */}
                        ...
                        {notifications.length === 0 ? (
                            ...
                            notifications.map((notif) => (
                                ...
                                >
                                    ...
                                    <Typography variant="subtitle1" sx={{ fontWeight: notif.status === NotificationStatusEnum.UNREAD ? "bold" : "normal" }} whiteSpace={"normal"}>
                                        {notif.message}
                                    </Typography>

                                    {/* Timestamp */}
                                    <Typography variant="caption" sx={{ color: "gray", alignSelf: "flex-end" }}>
