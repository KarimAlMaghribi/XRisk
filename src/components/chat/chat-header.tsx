...
import {selectRiskById, selectStatusById} from "../../store/slices/risks/selectors";
...
export const ChatHeader = () => {
    ...
    useEffect(() => {
        const mappedProfile: Publisher = {
            ...
            address: `${opposingProfile?.street || ""} ${opposingProfile?.number || ""} ${opposingProfile?.zip || ""} ${opposingProfile?.city || ""}`,
    ...
    useEffect(() => {
        if (uid === activeChat?.riskProvider?.uid) {
            activeChat?.riskTaker?.uid && dispatch(fetchUserProfileById(activeChat?.riskTaker?.uid));
        } else {
            activeChat?.riskProvider?.uid && dispatch(fetchUserProfileById(activeChat?.riskProvider?.uid));
        }
        ...
        <Box>
            {
                ...
                <Box>
                    <Box display="flex" alignItems="center" p={2}>
                        ...
                        <Box sx={{flexGrow: 1, textAlign: 'center'}} onClick={() => setOpenRiskDetails(true)}>
