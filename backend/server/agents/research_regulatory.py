"""
xrisk - Regulatory Research Agent
Author: Manuel Schott

Research Regulatory for xrisk application
Conducts research on legal and regulatory aspects
"""
...
class ResearchRegulatory(AIAgent):
    """Agent verantwortlich für die Recherche regulatorischer und rechtlicher Aspekte"""
    
    def __init__(self, api_key: str):
        super().__init__(api_key)
        self.research_type = "regulatory"
    ...
    def research_regulatory(self, risk_description: str, risk_type: str = "allgemein") -> Dict:
        """
        Recherchiert regulatorische und rechtliche Aspekte
        ...
        Returns:
            Dict: Regulatorische Rechercheergebnisse mit Kontextserver-Integration
        """
        messages = [
            {
                "role": "system",
                "content": """Sie sind ein Experte für Versicherungsrecht und regulatorische Anforderungen.
Ihre Aufgabe ist es, regulatorische und rechtliche Aspekte für ein spezifisches Risiko zu recherchieren.

WICHTIG: Halten Sie Ihre Antwort präzise - verwenden Sie weniger als """ + str(Config.AGENT_MAX_TOKENS.get('research_regulatory', 3000)) + """ Tokens.

Recherchieren Sie:
- Aktuelle Gesetze und Verordnungen
- Regulatorische Anforderungen und Compliance
- Rechtliche Haftungsfragen
- Versicherungsaufsichtsrechtliche Bestimmungen
- Internationale Standards und Richtlinien

Antworten Sie mit einem JSON-Objekt:
{
    "sources": ["Liste der verwendeten Rechtsquellen"],
    "findings": "Detaillierte Beschreibung der regulatorischen Analyse",
    "tags": ["maximal 3 präzise Tags für die Kategorisierung"]
}

WICHTIG für Tags:
- Generieren Sie maximal 3 präzise, aussagekräftige Tags
- Verwenden Sie spezifische Begriffe (z.B. "recht", "compliance", "haftung")
- Fokussieren Sie auf rechtliche/regulatorische Aspekte
- Tags sollen die Suche und Kategorisierung optimieren"""
            },
            {
                "role": "user",
                "content": f"Recherchieren Sie regulatorische und rechtliche Aspekte für dieses Risiko: {risk_description}"
            }
        ]
        ...
        try:
            ...
            result.update({
                ...
                "methodology": "Rechtliche und regulatorische Rahmenanalyse"
            })
            ...
            raise Exception(f"Research regulatory failed: {str(e)}")
    ...
    def _store_research_result(self, research_result: Dict, risk_type: str, risk_description: str) -> Optional[int]:
        ...
        try:
            ...
            if ai_tags and len(ai_tags) <= 3:
                ...
                tags = [risk_type, "regulatory"]
                if ai_tags:
                    tags.extend(ai_tags[:1])
                tags = tags[:3]
            
            context_data = {
                "source": f"Research Agent - Regulatory ({', '.join(research_result.get('sources', ['KI-Analyse'])[:2])})",
            ...
            # MCP expects lowercase table names (e.g., 'kfz_regulatory')
            normalized_type = (risk_type or "allgemein").strip().lower()
            table_name = f"{normalized_type}_regulatory"
